...
class BpmnBuilder:
    def __init__(self, process_id: str, nodes: dict[str, Any], edges: list[tuple[str, str]], metadata: dict[str, Any] | None = None, start_event_classical_nodes: list[Any] | None = None, containsPlaceholder: bool = False):
        """
    ...
    def _init_xml(self) -> None:
        ...
        self.defs = ET.Element(
            self.qn(BPMN2_NS, "definitions"),
            {
                ...
                self.qn(XSI_NS, "schemaLocation"): "http://www.omg.org/spec/BPMN/20100524/MODEL BPMN20.xsd",
            },
        )
        self.process = ET.SubElement(self.defs, self.qn(BPMN2_NS, "process"), {"id": f"Process_{self.process_id}", "isExecutable": "true"})
    ...
    def _create_start_event(self) -> None:
        ...
        start_event = ET.SubElement(self.process, self.qn(BPMN2_NS, "startEvent"), {"id": self.start_id})
        ext = ET.SubElement(start_event, self.qn(BPMN2_NS, "extensionElements"))
        form = ET.SubElement(ext, self.qn(CAMUNDA_NS, "formData"))
        ET.SubElement(form, self.qn(CAMUNDA_NS, "formField"), {"id": "ipAdress", "label": "IP Adresse", "type": "string", "defaultValue": "192.168.178.65"})
    ...
    def _create_service_task(self, task_id: str, name: str, connector_payload_script: str | None = None, connector_output_var: str | None = None, connector_output_script: str | None = None, extra_inputs: dict[str, str] | None = None, extra_outputs: dict[str, str] | None = None, async_after: bool = True, exclusive: bool = False) -> None:
        ...
        if async_after: attrs[self.qn(CAMUNDA_NS, "asyncAfter")] = "true"
        if exclusive: attrs[self.qn(CAMUNDA_NS, "exclusive")] = "true" # usually false in code unless setCircuit
        else: attrs[self.qn(CAMUNDA_NS, "exclusive")] = "false"
        ...
        if connector_payload_script or extra_inputs or extra_outputs:
            ...
            ET.SubElement(header_map, self.qn(CAMUNDA_NS, "entry"), {"key": "Accept"}).text = "application/json"
            ET.SubElement(header_map, self.qn(CAMUNDA_NS, "entry"), {"key": "Content-Type"}).text = "application/json"
            ...
            if connector_output_var and connector_output_script:
                 out_param = ET.SubElement(io, self.qn(CAMUNDA_NS, "outputParameter"), {"name": connector_output_var})
    ...
    def _create_script_task(self, task_id: str, name: str, script_content: str, async_after: bool = False, exclusive: bool = False) -> None:
        """Creates a Script Task with Groovy script content."""
        attrs = {"id": task_id, "name": name, "scriptFormat": "groovy"}
        if async_after: attrs[self.qn(CAMUNDA_NS, "asyncAfter")] = "true"
        if exclusive: attrs[self.qn(CAMUNDA_NS, "exclusive")] = "true"
    ...
    def _create_chain_common(self, start_node: str) -> tuple[str, str, str, str, str, str]:
        ...
        # Elements
        self._create_service_task(createdeploym_id, "Create Deployment", 
            GroovyScript.PAYLOAD_CREATE_DEPLOYMENT, "deploymentId", GroovyScript.OUTPUT_DEPLOYMENT_ID,
            extra_inputs={"method": "POST", "url": "http://${ipAdress}:8080/deployments/"})
            
        self._create_service_task(exejob_id, "Execute Job",
            GroovyScript.PAYLOAD_EXECUTE_JOB, "jobId", GroovyScript.OUTPUT_JOB_ID,
            extra_inputs={"method": "POST", "url": "http://${ipAdress}:8080/jobs/"})
            
        self._create_service_task(getjobres_id, "Get Job Results",
             extra_inputs={"method": "GET", "url": "http://${ipAdress}:8080${jobId}"},
             extra_outputs={"resultExecution": GroovyScript.OUTPUT_RESULT_EXECUTION, "statusJob": GroovyScript.OUTPUT_STATUS_JOB},
             async_after=False, exclusive=False) # Defaults in orig code? Check below... orig code didn't specify async on getjobres, so False.
             
        self._create_script_task(setvars2_id, "Set Variables", GroovyScript.SCRIPT_SET_VARS_2)
        
        ET.SubElement(self.process, self.qn(BPMN2_NS, "userTask"), {"id": human_task_id, "name": "Analyze Results"})
        self._create_exclusive_gateway(gateway3_id) # gateway_default_targets is always empty at this stage therefore there will be no default attribute 
        self._create_exclusive_gateway(gateway4_id) # gateway_default_targets is always empty at this stage therefore there will be no default attribute
        
        ET.SubElement(self.process, self.qn(BPMN2_NS, "userTask"), {"id": analyzefailedjob_id, "name": "Analyze Failed Job"})
        ET.SubElement(self.process, self.qn(BPMN2_NS, "endEvent"), {"id": altend2_id, "name": ""})

        return createdeploym_id, exejob_id, getjobres_id, setvars2_id, gateway3_id, gateway4_id
    ...
    def _create_chain_standard(self, start_node: str) -> None:
        ...
        # Common parts
        createdeploym_id, exejob_id, getjobres_id, setvars2_id, gateway3_id, gateway4_id = self._create_chain_common(start_node)
        
        # Specific parts
        self._create_script_task(setcirc_id, "Set Circuit", GroovyScript.SCRIPT_SET_CIRCUIT, async_after=True, exclusive=True)
    ...
    def _create_chain_placeholder(self, start_node: str) -> None:
        ...
        # Specific parts
        self._create_script_task(setvars1_id, "Set Variables", GroovyScript.SCRIPT_SET_VARS_1)
        
        self._create_service_task(backendreq_id, "Send Backend Request",
             GroovyScript.PAYLOAD_BACKEND_REQ, "uuid", GroovyScript.OUTPUT_UUID,
             extra_inputs={"method": "POST", "url": "http://${ipAdress}:8000/compile"})
             
        self._create_service_task(pollstat_id, "Poll Status",
             extra_inputs={"method": "GET", "url": "http://${ipAdress}:8000/status/${uuid}"},
             extra_outputs={"status": GroovyScript.OUTPUT_STATUS})
             
        self._create_service_task(retrievecirc_id, "Retrieve Circuit",
             extra_inputs={"method": "GET", "url": "http://${ipAdress}:8000/results/${uuid}"},
             extra_outputs={"circuit": GroovyScript.OUTPUT_CIRCUIT},
             async_after=False, exclusive=False) # check defaults
        
        self._create_exclusive_gateway(gateway1_id) # gateway_default_targets is always empty at this stage therefore there will be no default attribute
        self._create_exclusive_gateway(gateway2_id) # gateway_default_targets is always empty at this stage therefore there will be no default attribute
        
        self._create_script_task(updatevars_id, "Update Variables", GroovyScript.SCRIPT_UPDATE_VARS)
        ET.SubElement(self.process, self.qn(BPMN2_NS, "userTask"), {"id": analyzefailedtransf_id, "name": "Analyze Failed Transformation"})
        ET.SubElement(self.process, self.qn(BPMN2_NS, "endEvent"), {"id": altend1_id, "name": ""})
    ...
    def _calculate_layout(self, start_nodes: list[str], incoming: dict[str, list[str]], outgoing: dict[str, list[str]]) -> None:
        ...
        for nid in topo_order:
            if nid not in node_level:
                ...
                if predecessors:
                    node_level[nid] = max(node_level.get(p, 0) for p in predecessors) + 1
                else:
                    node_level[nid] = 1
        ...
        for chain_level, start_node in enumerate(start_nodes):
            ...
            if self.containsPlaceholder:
                ...
                self.task_positions[pollstat_id] = (x + 2 * (BPMN_TASK_WIDTH + BPMN_GAP_X), y)
                
                # Gateway 1
                self._place_gateway_between(backendreq_id, pollstat_id, gateway1_id)
                self.task_positions[retrievecirc_id] = (x + 3 * (BPMN_TASK_WIDTH + BPMN_GAP_X), y)
                
                # Gateway 2
                self._place_gateway_between(pollstat_id, retrievecirc_id, gateway2_id)
                self.task_positions[updatevars_id] = (x + 2 * (BPMN_TASK_WIDTH + BPMN_GAP_X), y - BPMN_TASK_HEIGHT - 30)
                
                gx2, gy2 = self.task_positions[gateway2_id]
                self.task_positions[analyzefailedtransf_id] = (gx2, gy2 + BPMN_TASK_HEIGHT + 20)
    ...
    def _create_sequence_flow(self, fid: str, src: str, tgt: str) -> None:
        ...
        sf = ET.SubElement(self.process, self.qn(BPMN2_NS, "sequenceFlow"), {
    ...
    def _create_diagram(self, start_nodes: list[str]) -> None:
        ...
        def add_shape(eid: str, x: Any, y, w, h):
            shape = ET.SubElement(plane, self.qn(BPMNDI_NS, "BPMNShape"), {"id": f"{eid}_di", "bpmnElement": eid})
            ET.SubElement(shape, self.qn(DC_NS, "Bounds"), x=str(x), y=str(y), width=str(w), height=str(h))
        
        diagram = ET.SubElement(self.defs, self.qn(BPMNDI_NS, "BPMNDiagram"), {"id": "BPMNDiagram_1"})
        plane = ET.SubElement(diagram, self.qn(BPMNDI_NS, "BPMNPlane"), {"id": "BPMNPlane_1", "bpmnElement": f"Process_{self.process_id}"})
        ...
        valid_bpmn_ids = {
            el.attrib["id"]
            for el in self.process.findall(".//*[@id]")
        }
        ...
        for eid, (x, y) in self.task_positions.items():
            if eid not in valid_bpmn_ids: continue
