...
from app.lib.ast_utils import annotate
...
class RenameRegisterTransformer(QASMTransformer[SectionInfo]):
    ...
    def visit_AliasStatement(
        ...
        return self.generic_visit(
            annotate(AliasStatement(identifier, node.value), node.annotations),
            context,
        )
    ...
    def visit_QubitDeclaration(
        ...
        return self.generic_visit(
            annotate(QubitDeclaration(identifier, node.size), node.annotations),
            context,
        )

    def visit_QuantumGateDefinition(
        self,
        node: QuantumGateDefinition,
        context: SectionInfo,
    ) -> QASMNode:
    ...
    def visit_IODeclaration(
        ...
        identifier = self.new_identifier(node.identifier, context)
        return annotate(
            IODeclaration(node.io_identifier, node.type, identifier),
            node.annotations,
        )

    def visit_CalibrationDefinition(
        self,
        node: CalibrationDefinition,
        context: SectionInfo,
    ) -> QASMNode:
        name = self.new_identifier(node.name, context)
        return self.generic_visit(
