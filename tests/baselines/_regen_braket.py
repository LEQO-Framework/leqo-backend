"""One-off regen helper for compile_braket baselines.

Run only when the Amazon Braket exporter output deliberately changes:

    .venv/Scripts/python.exe -m pytest tests/baselines/_regen_braket.py -s

Walks tests/baselines/compile_braket/, replays each request through the live
...
BASELINE_DIR = Path(__file__).parent / "compile_braket"
POLL_INTERVAL = 0.1
MAX_ATTEMPTS = 50
...
def test_regenerate_braket_baselines(client: TestClient) -> None:
